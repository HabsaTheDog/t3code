...
- A Moodle PDF is publishable only when its extraction \`run-summary.md\` is terminal \`success\` or \`partial\`, extraction \`error.log\` is empty, and the official render produces non-empty \`document.typ\` and \`document.pdf\` with an empty render \`error.log\`. Never call \`render\` on a failed extraction, never publish a PDF generated from an incomplete handoff, and never describe such a file as ready merely because it exists.
