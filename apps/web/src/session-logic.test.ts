...
describe("derivePendingApprovals", () => {
  ...
  it("keeps subagent provenance on root-visible pending approvals", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        kind: "approval.requested",
        summary: "Command approval requested",
...
describe("deriveWorkLogEntries", () => {
  ...
  it("keeps historical entries and their turn ids", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "turn-1",
        turnId: "turn-1",
        summary: "First tool call complete",
        kind: "tool.completed",
      }),
      makeActivity({
        id: "turn-2",
        turnId: "turn-2",
        summary: "Second tool call complete",
        kind: "tool.completed",
      }),
    ...
    expect(entries.map((entry) => [entry.id, entry.turnId])).toEqual([
      ["turn-1", TurnId.make("turn-1")],
      ["turn-2", TurnId.make("turn-2")],
    ]);
  });
  ...
  it("quotes argv backslashes safely without changing ordinary Windows paths", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "command-tool-quoted-windows-path",
        kind: "tool.completed",
        summary: "Ran command",
        payload: {
          itemType: "command_execution",
          data: {
            item: {
              command: ["tool", 'C:\\Program Files\\folder\\"quoted"\\'],
            },
          },
        },
      }),
    ...
    const [entry] = deriveWorkLogEntries(activities);
    expect(entry?.command).toBe('tool "C:\\Program Files\\folder\\\\\\"quoted\\"\\\\"');
  });
