...
const PROVIDER_ORDER: readonly ProviderSetupProvider[] = ["codex", "claude", "cursor", "opencode"];
...
function getJobStatusCopy(job: ProviderJob, providerLabel: string): {
  title: string;
  detail: string;
...
export function ProviderSetupStep() {
  ...
  const [secretValues, setSecretValues] = useState<Partial<Record<ProviderSetupProvider, string>>>(
    {},
  );
  ...
  const persistEnvironmentSecret = async (
    ...
    const currentInstance = settings.providerInstances[instanceId] ?? ({
      driver,
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {PROVIDER_ORDER.map((providerName) => {
          ...
          const providerLabel =
            capability?.displayName ?? providerName.charAt(0).toUpperCase() + providerName.slice(1);
          const ProviderIcon = PROVIDER_ICON_BY_NAME[providerName];
          const statusCopy = job ? getJobStatusCopy(job, providerLabel) : null;
          const installJustCompleted = job?.status === "completed" && job.action.kind === "install";
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {!status.installed ? <Badge variant="outline">Install required</Badge> : null}
                      </div>
                ...
                <div className="mt-5 grid gap-2">
                  ...
                  {retryAction ? (
                    <Button
                      ...
                      disabled={retryAction.secretInput !== null && !secretValues[providerName]?.trim()}
      ...
      <Dialog open={apiKeyDialog !== null} onOpenChange={(open) => (!open ? setApiKeyDialog(null) : null)}>
